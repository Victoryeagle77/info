...
/*
2) - La variable p est un pointeur est initialisé 0, et son type (void*) est converti vers n'importe quel type d'adresse.
L'opérateur * permet de manipuler p à travers son adresse plutôt que son nom.
- Le programme possède une boucle ou les variable min et maj correspondent respectivement à la lettre 'a' en minuscule et 'A', en majuscule.
...
-> Or sachant que l'adresse de p est 0 (NULL), elle ne correspondra pas à l'adresse de la variable min de type char,
...
-> Et ainsi de suite ... 
-> Donc le programme affichera à l'exécution l'alphabet avec une lettre sur deux en minuscule puis en majuscule, ce qui donnera ceci : 
