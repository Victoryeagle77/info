...
(or sachant que q = l'adresse de c, alors *q pointe vers la valeur à laquelle est attribuée cette adresse soit le pointeur *q = 3 + 1).
- p = l'adresse de q.
- q = l'adresse de b donc, pour l'instruction précédente, p = l'adresse de b.
- Le pointeur *p = le pointeur *p - le pointeur *q (avec le pointeur *p = 4 et le pointeur *q = b = 2) donc le pointeur *p = 4 - 2 = 2.
...
- Le pointeur *q = le pointeur *p / le pointeur *q (avec le pointeur *p = 24 et le pointeur *q = 6) donc 24 / 4 = 6.
