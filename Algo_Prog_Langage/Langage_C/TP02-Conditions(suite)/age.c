...
int main(void){
	...
	printf("Date actuelle : %.2d / %.2d / %.4d\n", j_c, m_c, a_c);
        // On utilise la varaible m en tant que variable tampon récupérant une nouvelle valeur qui sera un calcul avec elle-même.
	m = m_c - m;
	// Pareil pour a qui sera l'age final.
	a = a_c - a;
	// La soustraction entre les 2 années dépend de celle entre les 2 mois, et elle même dépend de celle entre les 2 jours.
