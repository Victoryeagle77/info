...
Lorsqu'une compagnie d'assurance prend en charge des réparations, 
elle laisse une franchise à la charge de l'assuré.
...
Écrivez un programme qui demande le montant total des réparations, 
puis affiche la franchise correspondante ainsi que la part prise en charge par l'assurance. 
*/
...
int main(void){
  ...
  if(franchise < MIN){
    if(montant_rep > MIN) { franchise = MIN; }
    else { franchise = montant_rep; }
  } else if(franchise > MAX) { franchise = MAX; }
