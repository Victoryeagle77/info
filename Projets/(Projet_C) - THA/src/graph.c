...
static void repere_graph(Display *d, const Window f, unsigned int abscisse, 
    ...
    XDrawString(d, f, style(d, 0xffaaaa), 20, ordonnee/2.0 + 50, str, strlen(str));
    ...
    XDrawString(d, f, style(d, 0xaaccff), abscisse/2.0 + 30, 30, str, strlen(str));
...
static void repere_model(Display *d, const Window f, unsigned int abscisse, 
    ...
    XDrawString(d, f, style(d, 0xffccff), 15, ordonnee/2.0 + 40, "Time (in s)", 11);
...
static void tracer_fonction(Display *d, const Window f, volatile unsigned short int x,
    ...
    /* Indice du coefficient directeur */
