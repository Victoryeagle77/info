...
int jeu(void){
  ...
  while (continuation) {
    choix = 0;
    SourisPosition();
    /* Coordonnées de la zone cliquable "Joueur" */
    choix = interaction(_X, _Y, commencer);
    if (!choix) {
      /* Coordonnées de la zone cliquable "quitter" */
  ...
  while (continuation) {
    tampon = 0;
    SourisPosition();
    /* Placement des couleurs dans les cases */
    if (SourisCliquee()) {
    ...
    if (ToucheEnAttente()) {
      if (Touche() == XK_Return) {
        if (cases[index][0][7] != 0 && cases[index][1][7] != 0 
          ...
          index++;
          /* Fin de partie */
          if (validation_case[0] == 4) { /* Victoire */
            index = -1;
            ChargerImage("../images/victoire.png", 100, 360, 0, 0, 250, 220);
            sleep(3);
            FermerGraphique();
            exit(0);
          } else if (index >= 10) { /* Défaite */
