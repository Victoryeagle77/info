...
extern const GC style(Display *d, const char *nom, const unsigned int couleur){
    static XGCValues gcv;
    /* Structure de la police de texte */
    static XFontStruct *police;
    /* Chargement de la police */
    police = config_police(d, nom);
    ...
    return XCreateGC(d, DefaultRootWindow(d), GCForeground | GCBackground 
                        | GCLineWidth | GCLineStyle |GCFont, &gcv);
}
...
static const XContext config_generale(Display * d, volatile unsigned short int x, 
   ...
   volatile Zone *src = calloc(sizeof(*src), 1);
   ...
   creer_bouton(d, f, "Exit", police, (x/2)-40, (y-150), 80, 
                  (police->ascent + police->descent) * 2 + 5,
                  0x803030, 0x404040, cx, quitter, data);

   creer_graph(d, f, police, (x*30/100)-50, (y*20/100), x/2, y/2, cx);
...
static void affichage(const Zone *canvas, const XEvent *e, 
   ...
   static char str[2];
   /* Bouton de sortie */
   if((canvas->texte) && (!(canvas->validation))){
      /* Texte du bouton */
   ...
   /* Canvas graphique */
   }else if(canvas->validation)
      /* Tracage du graphique */
      tracer_fonction(e->xany.display, e->xany.window, x, y);
   /* Fond */
   else{
