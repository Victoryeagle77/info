...
/* Remplissage dans le tableau de coordonnées de la carte */
  ...
  for(i=1;i<(k-1);i++){
    for(j=1;j<(l-1);j++){
      if(tableau[i][j].niveau < c){ 
        /* Si le niveau est inferieur à celui indiquée, alors le niveau deviens égal 
...
terrain** Remplissage(terrain** tableau, int k, int l){
  ...
  /* "Nettoyage" ou "lissage" des niveaux pour plus de cohérence */
  return tableau;
}