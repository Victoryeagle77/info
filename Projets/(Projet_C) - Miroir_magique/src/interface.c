...
static void affichage(Display *d, const Zone *canvas, const XEvent *e, XImage *ximage, Window *zone,
   ...
   /* Fond */
      ...
      XGetGeometry(d, e->xany.window, &racine, &hauteur, &largeur, &abscisse, 
                     &ordonnee, &bordure, &profondeur);

      /* Image de fond */
      XPutImage(d, e->xany.window, GC(d), ximage, 0, 0, 0, 0, abscisse, ordonnee);
      /* Affichage du titre */
      XDrawString(d, e->xany.window, GC(d), (abscisse/2)-150, 50, "Temperature & Humidity Analyser", 31);
